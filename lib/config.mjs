...
export function configureHermes() {
  ...
  const existingApiKey = String(config.providers?.arcox?.api_key || '')
  ...
  config.tools.tool_search = typeof config.tools.tool_search === 'object' && !Array.isArray(config.tools.tool_search)
    ...
    : {}
  // Expose every enabled Hermes and MCP tool directly to the ARCOX model.
  // Tool-search remains useful for very small models, but it hid most tools
  // from the provider used by ARCOX AI Router.
