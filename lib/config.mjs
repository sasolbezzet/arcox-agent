...
export function parseConnectionInput(input, fallbackUrl = 'https://arcoxdex.vercel.app/mcp') {
  ...
  const url = String(urlMatch?.[1] || directUrlMatch?.[0] || fallbackUrl).replace(/[),.;]+$/, '')
  validateMcpUrl(url)
...
export async function probeMcpConnection(url, token) {
  ...
  // The connection token is bound to one Agent Wallet MSCA on the server.
  // Read its status through MCP so a local EOA or legacy Agent Jobs profile
  // can never be mistaken for the connected wallet.
  const statusCall = await mcpRequest(normalizedUrl, token, {
    jsonrpc: '2.0', id: 3, method: 'tools/call',
    params: { name: 'arcox_session_status', arguments: {} },
  }, second.sessionId || first.sessionId)
  const status = parseToolText(statusCall.payload)
  const walletAddress = String(status?.walletAddress || '').trim()
  if (!/^0x[0-9a-fA-F]{40}$/.test(walletAddress)) {
    throw new Error(status?.message || 'Remote MCP tidak mengembalikan Agent Wallet MSCA yang aktif')
  }
  if (status.active !== true) throw new Error(status.message || 'Agent Wallet MSCA belum aktif')
  return {
    ok: true,
    status: second.status,
    tools: tools.length,
    sessionId: statusCall.sessionId || second.sessionId || first.sessionId || '',
    walletAddress,
    walletType: 'MSCA',
    active: true,
  }
...
export function parseRpcResponse(text) {
  ...
  // Streamable HTTP commonly returns SSE records such as:
  // event: message\\n data: {"jsonrpc":"2.0",...}\\n\\n
  // Accept CRLF/LF, optional whitespace after `data:`, and multiple records.
  const candidates = raw
    .split(/\r?\n/)
    .filter(line => /^\s*data\s*:/i.test(line))
    .map(line => line.replace(/^\s*data\s*:/i, '').trim())
    .filter(value => value && value !== '[DONE]' )
  for (const candidate of [...candidates, raw.trim()]) {
    try {
      const parsed = JSON.parse(candidate)
      if (parsed && typeof parsed === 'object') return parsed
    } catch { /* ignore SSE comments, event names, and keepalive records */ }
  }
...
function parseToolText(payload) {
  ...
  if (!text) throw new Error('MCP status tool tidak mengembalikan data')
  try {
    return JSON.parse(text)
  } catch {
    throw new Error('MCP status tool mengembalikan data yang tidak valid')
  }
}
